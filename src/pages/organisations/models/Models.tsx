...
+					<div className="flex flex-1 overflow-y-auto">
 ...
 function ModelsPageHeader({ setModelsFilter }: { setModelsFilter: (val: string) => void }) {
 	...
 	return (
 		<PageHeader
 			title="Models"
+			subtitle="Manage your 3D models and assets"
 			beforeTitleElement={
-				<div className="flex">
...
+						<input
+							type="text"
+							className="w-64 rounded-lg border border-gray-300 bg-white py-2 pl-10 pr-4 text-sm transition-colors focus:border-red-500 focus:ring-2 focus:ring-red-500/20"
...
+							<input
+								type="text"
+								className="w-full rounded-lg border border-gray-300 bg-white py-2 pl-10 pr-4 text-sm transition-colors focus:border-red-500 focus:ring-2 focus:ring-red-500/20"
+								placeholder="Search models..."
+								onChange={(e) => setModelsFilter(e.target.value)}
+								aria-label="Filter models"
+							/>
...
+						<div className="flex items-center gap-2 border-b border-gray-100 px-4 py-3 text-sm text-gray-600 lg:px-6">
...
+										className="rounded px-1 py-0.5 transition-colors hover:bg-gray-100 hover:text-gray-900"
 										onClick={() => setCategory(b.id)}
 									>
-										{b.name === "root" ? "" : b.name }
+										{b.name === "root" ? organisation.name : b.name}
 									</button>
...
+						className="relative flex-1 overflow-y-auto p-4 lg:p-6"
 					>
...
+								<IconCube className="mb-4 h-16 w-16 text-gray-300" />
+								<h3 className="text-lg font-semibold text-gray-900">No models found</h3>
+								<p className="mt-1 text-sm text-gray-600">
+									{modelsFilter 
+										? "Try adjusting your search terms or category filter"
+										: "Get started by adding your first 3D model"
+									}
+								</p>
+								{isAdmin && !modelsFilter && (
+									<Button
+										className="mt-4"
+										onClick={() =>
+											pushModal(
...
+      className="group relative flex cursor-pointer flex-col overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm transition-all duration-200 hover:shadow-lg"
       variants={variants}
       whileHover="hover"
-      whileTap="click"
+      whileTap="tap"
...
+        }
+      }}
+      aria-label={`Open model ${data.name}`}
     >
       {data.thumbnailUrl ? (
         <img
...
+            className="absolute left-1/2 top-1/2 h-16 w-16 -translate-x-1/2 -translate-y-1/2 text-gray-400"
             strokeWidth={1}
-            color="#4c525c"
           />
         </div>
...
+                className="rounded-md bg-white/90 px-2 py-1 text-xs font-medium text-gray-700 shadow-sm backdrop-blur-sm"
               >
                 {type}
               </li>
             ))}
+          {files && files.length > 3 && (
+            <li className="rounded-md bg-white/90 px-2 py-1 text-xs font-medium text-gray-700 shadow-sm backdrop-blur-sm">
+              +{files.length - 3}
+            </li>
+          )}
         </ul>
